...
global user_id_list, user_id_names, user_id_codes, user_send_cats, id_send_cat
...
def save():
    ...
    for name, cat, code in zip(user_id_names, user_send_cats, user_id_codes):
        cat_str = ','.join(cat)
        f.write(f'{name} {cat_str} {code}\n')
    f.close()
...
def input_info():
    global user_id_list, user_id_names, user_id_codes, user_send_cats, id_send_cat, window

    ### init ###
    user_id_list, user_id_names, user_id_codes, user_send_cats = [], [], [], []
    ...
    for v in user_id_list:
        user_id_name, user_send_cat, user_id_code = v.split()
